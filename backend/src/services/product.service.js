...
export const createProductService = async (
  ...
  const { name, description, price, inStock, color, sizes, category } =
    productData;

  if (
    [name, description, price, inStock, color, sizes, category].some(
  ...
  for (const path of imagePaths) {
    ...
    if (result && result.url) {
      ...
      throw new ApiError(
        500,
        "Error uploading one or more images to Cloudinary"
      );
    }
  }
...
export const getAllProductsService = async (queryParams) => {
  ...
  const cacheKey = `products:${JSON.stringify(filters)}:${JSON.stringify(
    sortOptions
  )}:${page}:${limit}`;
  ...
  if (cachedProducts) {
    try {
      ...
      // Data in cache is not valid JSON, treat as cache miss
    }
  }
