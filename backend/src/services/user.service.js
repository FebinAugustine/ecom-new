...
export const registerUserService = async (userData) => {
  ...
  if (
    [email, fullname, password, phone].some((field) => field?.trim() === "")
  ) {
  ...
  const existingUserByPhone = await userRepository.findUserByPhone(phone);
...
export const loginUserService = async (loginData) => {
  ...
  // Store refresh token in Redis using the correct options object
  await redisClient.set(user._id.toString(), refreshToken, {
    EX: 7 * 24 * 60 * 60, // Set expiry in seconds (7 days)
  });

  // 3. Save the new refresh token to the database AND Redis
...
export const refreshAccessTokenService = async (incomingRefreshToken) => {
  ...
  // FIX #1: Use the correct environment variable
  ...
  // FIX #2: Return the user object along with the tokens
...
export const forgotPasswordVerifyCodeService = async (
  email,
  code,
  newPassword
) => {
...
export const verifyEmailService = async (email, code) => {
  ...
  const isCodeExpired =
    new Date() > new Date(user.email_verification_code_expiry);
